...
export class VueRuntimeGenerator extends RuntimeGenerator {
  ...
  protected processPlatformTypes(): void {
    ...
    this.writeLine(
      `export interface ${instance} extends Vue, ${this.getInterfaceName(
        'props'
      )} {`
    )
    this.indent(1)
    ...
    this.writeLine(
      `${this.prefix.toUpperCase()}_CONFIGS?: ${this.getInterfaceName(
        'config'
      )};`
    )
    this.indent(-1)
    ...
    this.writeLine(
      `export type ${this.getTypeName('options')} = ` +
        ...
        `DefaultComputed, ArrayPropsDefinition<${this.getInterfaceName(
          'props'
        )}>, ` +
    ...
    this.writeLine(
      `export type ${this.getTypeName('Render')} = ` +
        `(h: ${this.getTypeName('helper')}, props: ${this.getInterfaceName(
          'props',
          true
        )}) => VNode;`
    )
  ...
  protected processPlatformWrapper(): void {
    ...
    // 渲染函数
    this.writeLine(
      `render(this: ${this.getInterfaceName(
        'instance'
    ...
    this.writeLine('spin,')
    this.writeLine(
      `${this.prefix.toUpperCase()}_CONFIGS = DEFAULT_${this.prefix.toUpperCase()}_CONFIGS`
    )
    this.indent(-1)
    this.writeLine('} = this;')
    this.writeLine()
